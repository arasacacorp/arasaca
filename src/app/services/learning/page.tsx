...
export default function LearningPage() {
  return (
    <main className="min-h-screen flex flex-col" style={{ background: C.muted }}>
          ...
          <div className="flex flex-col gap-8 lg:flex-row lg:items-center lg:justify-between py-6 md:py-10 lg:py-12">
            ...
            <motion.div className="flex-1" variants={fadeUp} initial="hidden" animate="visible" custom={0.1}>
              ...
              >
                ...
                <ChevronRight className="w-3 h-3" style={{ color: "rgba(255,255,255,0.25)" }} />
                <span className="text-[12px] font-medium" style={{ color: C.mint }}>Обучение и развитие</span>
              </motion.nav>
              ...
              >
                Корпоративные программы развития, бережливое производство, управление знаниями и собственная Корпоративная академия Арасаки для непрерывного развития организации.
              </motion.p>
      ...
      {/* ═══════════════════════════════════════════════════
          DIRECTIONS — Bento grid for 4 items (2x2)
          ═══════════════════════════════════════════════════ */}
        ...
        <div className="container-kept">
          <motion.div className="mb-12 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between" variants={fadeUp} initial="hidden" whileInView="visible" viewport={vp}>
            <div>
              ...
              <h2 className="heading-section" style={{ color: C.textDark }}>Направления обучения и развития</h2>
            </div>
            <p className="text-[14px] font-normal leading-relaxed max-w-xs lg:text-right" style={{ color: C.textMuted }}>Четыре ключевых направления<br />для развития компетенций организации</p>
          </motion.div>

          <div className="grid grid-cols-2 gap-3 sm:grid-cols-2 md:gap-4">
            {learningSubDirections.map((direction, index) => {
              const IconComponent = iconMap[direction.icon] || GraduationCap;
              const isOrange = direction.slug === "arasaca-academy";
              ...
              return (
                <motion.div key={direction.id} className="flex" variants={scaleIn} initial="hidden" whileInView="visible" viewport={vp} custom={index * 0.08}>
                  <Link href={`/services/learning/${direction.slug}`} className="group block h-full w-full">
                    <motion.div
                      className={cn("relative flex flex-col justify-between overflow-hidden rounded-lg p-4 md:p-5 transition-all duration-300 h-full", isOrange ? "group-hover:shadow-lg" : "border shadow-sm group-hover:shadow-md")}
                      style={{ background: isOrange ? C.orange : C.white, color: isOrange ? "#ffffff" : C.textDark, borderColor: isOrange ? undefined : C.border, minHeight: "180px" }}
                      whileHover={{ y: -3, transition: { duration: 0.25 } }}
                    >
                      {isOrange && (
                        <div className="pointer-events-none absolute inset-0 opacity-[0.06]" style={{ backgroundImage: `linear-gradient(135deg, transparent 25%, rgba(255,255,255,0.3) 25%, rgba(255,255,255,0.3) 50%, transparent 50%, transparent 75%, rgba(255,255,255,0.3) 75%)`, backgroundSize: "20px 20px" }} />
                      )}
                      {!isOrange && (
                      ...
                      <div className="pointer-events-none absolute left-0 top-0 h-full w-1" style={{ background: isOrange ? "rgba(255,255,255,0.4)" : C.dna }} />
                      ...
                      <div className="relative z-10 flex justify-end mt-3">
                        <div className="flex h-8 w-8 items-center justify-center rounded-full transition-all duration-300 group-hover:translate-x-0.5 group-hover:-translate-y-0.5" style={{ background: isOrange ? "rgba(255,255,255,0.2)" : "rgba(0,140,149,0.1)" }}>
                          <ArrowRight className="h-4 w-4" style={{ color: isOrange ? "#ffffff" : C.dna }} />
                        </div>
                      </div>
                    </motion.div>
                  </Link>
        ...
        <div className="container-kept">
          <motion.div className="mb-14 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between" variants={fadeUp} initial="hidden" whileInView="visible" viewport={vp}>
            ...
            <p className="text-[14px] font-normal leading-relaxed max-w-sm lg:text-right" style={{ color: "rgba(255,255,255,0.5)" }}>
              От диагностики до внедрения: создаём системы обучения, развиваем компетенции и строим культуру непрерывного развития.
            </p>
          </motion.div>
