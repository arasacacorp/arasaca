...
/* ═══════════════════════════════════════════════════════
   COLOUR PALETTE (same as services page)
   ═══════════════════════════════════════════════════════ */
const C = {
  dna: "#008C95",
  dnaHover: "#007a82",
...
export default function ConsultingPage() {
  return (
    <main className="min-h-screen flex flex-col" style={{ background: C.muted }}>
          ...
          <div className="flex flex-col gap-8 lg:flex-row lg:items-center lg:justify-between py-6 md:py-10 lg:py-12">
            ...
            >
              ...
              >
                ...
                <ChevronRight className="w-3 h-3" style={{ color: "rgba(255,255,255,0.25)" }} />
                <span className="text-[12px] font-medium" style={{ color: C.mint }}>Консалтинг</span>
              </motion.nav>
              ...
              >
                Комплексные решения для стратегического развития, финансового моделирования, операционной эффективности и&nbsp;управления проектами. Помогаем принимать обоснованные решения и&nbsp;достигать измеримых результатов.
              </motion.p>
      ...
      {/* ═══════════════════════════════════════════════════
          DIRECTIONS — Consulting sub-directions with unique card design
          ═══════════════════════════════════════════════════ */}
        ...
        <div className="container-kept">
          ...
          >
            ...
            <p className="text-[14px] font-normal leading-relaxed max-w-xs lg:text-right" style={{ color: C.textMuted }}>
              Семь ключевых направлений для<br />комплексного развития вашего бизнеса
            </p>
          </motion.div>

          {/* Cards Grid — Bento layout: white cards + orange highlight */}
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:gap-4">
            {consultingSubDirections.map((direction, index) => {
              const IconComponent = iconMap[direction.icon] || Target;
              const isOrange = direction.slug === "government-support";
              // Bento: row2 — orange spans 2 cols, row3 — last card spans 2 cols
              const isWide = index === 3 || index === 6;

              return (
                <motion.div
                  key={direction.id}
                  className={cn("flex", isWide && "sm:col-span-2 lg:col-span-2")}
                  variants={scaleIn}
                  initial="hidden"
                  whileInView="visible"
                  viewport={vp}
                ...
                >
                  <Link
                    href={`/services/consulting/${direction.slug}`}
                    className="group block h-full w-full"
                  >
                    <motion.div
                      className={cn(
                        "relative flex flex-col justify-between overflow-hidden rounded-lg p-4 md:p-5 transition-all duration-300 h-full",
                        isOrange
                          ? "group-hover:shadow-lg"
                          : "border shadow-sm group-hover:shadow-md",
                      )}
                      style={{
                        background: isOrange ? C.orange : C.white,
                        color: isOrange ? "#ffffff" : C.textDark,
                        borderColor: isOrange ? undefined : C.border,
                        minHeight: "180px",
                      }}
                      whileHover={{ y: -3, transition: { duration: 0.25 } }}
                    >
                      {/* Decorative pattern for orange card */}
                      {isOrange && (
                        <div
                      ...
                      {/* Decorative dots for white cards */}
                      {!isOrange && (
                        <div
                      ...
                      <div
                        ...
                        style={{ background: isOrange ? "rgba(255,255,255,0.4)" : C.dna }}
                      />
                      ...
                      <div className="relative z-10">
                        ...
                        {/* Badge for government support */}
                        {isOrange && (
                          <span
                            className="inline-flex items-center gap-1 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider mt-1"
                            style={{
                      ...
                      <div className="relative z-10 flex justify-end mt-3">
                        <div
                          ...
                          style={{ background: isOrange ? "rgba(255,255,255,0.2)" : "rgba(0,140,149,0.1)" }}
                        >
                          <ArrowRight
                            className="h-4 w-4"
                            style={{ color: isOrange ? "#ffffff" : C.dna }}
                          />
                        </div>
                      </div>
                    </motion.div>
        ...
        <div className="container-kept">
          ...
          >
            ...
            <p className="text-[14px] font-normal leading-relaxed max-w-sm lg:text-right" style={{ color: "rgba(255,255,255,0.5)" }}>
              От диагностики до внедрения — сопровождаем на каждом этапе. Комплексный подход позволяет достигать измеримых результатов и обеспечивать устойчивое развитие бизнеса.
            </p>
          </motion.div>
            ...
            {[
              {
                icon: Target,
                title: "Стратегия",
                description: "От диагностики до дорожной карты и внедрения",
                step: "01",
              },
              {
                icon: BarChart3,
                title: "Управление",
                description: "Проекты, портфели и трансформационные программы",
                step: "02",
              },
              {
                icon: ClipboardList,
                title: "Реализация",
                description: "Сопровождение от идеи до измеримого результата",
                step: "03",
              },
            ].map((item, index) => (
              <motion.div
