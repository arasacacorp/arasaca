...
export interface ServiceSlugPageConfig {
  ...
  parent: {
    title: string;            // Хлебная крошка: "Консалтинг"
    href: string;             // URL: "/services/consulting"
    badgeText: string;        // Badge: "Направление консалтинга"
    otherTitle: string;       // "Другие направления консалтинга"
    allLinkText: string;      // "Все направления консалтинга"
  };
}
...
export default function ServiceSlugPageTemplate(config: ServiceSlugPageConfig) {
  ...
  return (
    <main className="min-h-screen flex flex-col" style={{ background: C.muted }}>
          ...
          <div className="flex flex-col gap-8 lg:flex-row lg:items-center lg:justify-between py-6 md:py-10 lg:py-12">
            <motion.div className="flex-1" variants={fadeUp} initial="hidden" animate="visible" custom={0.1}>
              <motion.nav className="flex items-center gap-2 mb-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, delay: 0.1 }}>
                ...
                <ChevronRight className="w-3 h-3" style={{ color: "rgba(255,255,255,0.25)" }} />
                <Link href={parent.href} className="text-[12px] transition-colors hover:text-white" style={{ color: "rgba(255,255,255,0.4)" }}>{parent.title}</Link>
                <ChevronRight className="w-3 h-3" style={{ color: "rgba(255,255,255,0.25)" }} />
              ...
              <motion.h1
                ...
                style={{ fontFamily: "var(--font-russo)", fontSize: "clamp(1.75rem, 4vw, 3rem)", fontWeight: 700, lineHeight: 1.25, letterSpacing: "-0.01em", color: C.white }}
        ...
        <div className="container-kept">
          <div className="grid lg:grid-cols-[280px_1fr] gap-8 lg:gap-16">
            <div className="lg:sticky lg:top-32 lg:self-start">
              ...
              <p className="text-[13px] leading-relaxed" style={{ color: C.textMuted }}>
                Полный перечень услуг в рамках {direction.shortTitle.toLowerCase()}
              </p>
            </div>
      ...
      {/* ═══════════════════════════════════════════════════
          OTHER DIRECTIONS — Dark background carousel
          ═══════════════════════════════════════════════════ */}
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            <div className="flex items-center justify-between mb-10">
              <div>
                ...
                <h2 className="mt-3" style={{ fontSize: "clamp(1.5rem, 3vw, 2.25rem)", fontWeight: 700, lineHeight: 1.25, color: C.white }}>
                  {parent.otherTitle}
                </h2>
              </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  onMouseEnter={(e) => { e.currentTarget.style.borderColor = C.dna; e.currentTarget.style.background = C.dna; e.currentTarget.style.color = "#fff"; }}
                  onMouseLeave={(e) => { e.currentTarget.style.borderColor = "rgba(255,255,255,0.15)"; e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = "rgba(255,255,255,0.6)"; }}
              ...
              {otherDirections.map((relDirection) => {
                ...
                return (
                  <motion.div key={relDirection.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4 }} className="flex-shrink-0 w-[280px]">
                    <Link
                      href={`${parent.href}/${relDirection.slug}`}
                      className="group block h-full rounded-xl p-6 transition-all duration-300"
                      style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)" }}
                      onMouseEnter={(e) => { e.currentTarget.style.background = "rgba(255,255,255,0.1)"; e.currentTarget.style.borderColor = "rgba(0,140,149,0.4)"; }}
            ...
            <div className="mt-10">
              <Link href={parent.href} className="inline-flex items-center gap-2 font-medium transition-all duration-300 hover:gap-3" style={{ color: C.mint }}>
                {parent.allLinkText}
