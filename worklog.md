...
Task ID: 5-extra
Agent: main
Task: Доработка шаблона /press-center/[slug] — hero фон, layout, сайдбар

Work Log:
- Hero фон: заменён gradient на C.dark + декоративные оверлеи (диагональные линии, оранжевый/бирюзовый glow) — по аналогии с ServiceSlugPageTemplate
- Hero низ: убран треугольник SVG, теперь ровный край
- Контент: шрифт уменьшен с text-base/lg:text-lg → text-sm/lg:text-[15px]
- Контент: ширина ограничена до 65% (lg:w-[65%])
- Добавлен сайдбар справа (35%): карточка «Запрос в редакцию» (тёмная, с email/phone) + «Последние публикации» (4 ссылки)
- Сайдбар sticky (lg:sticky lg:top-28)
- Обновлён page.tsx: передаётся latestPublications через props из getLatestPublications(5)
- Мобильная адаптация: сайдбар переносится под контент
- Agent Browser: 14/14 проверок пройдены
